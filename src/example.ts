...
import { ActionRow, Button, Container, MediaGallery, MediaGalleryItem, Section, Separator, TextDisplay, Thumbnail } from "./types";

const doRequest = async (url: string, method: string, data?: any) => {
    try {
...
doRequest("https://discord.com/api/v10/channels/1173457060332060752/messages", "POST", {
    ...
    console.error("Error:", error.response.data.errors.components[0].components[1]);
});